...
SWEP.InLoadoutFor = { ROLE_INNOCENT, ROLE_TRAITOR, ROLE_DETECTIVE, ROLE_MERCENARY, ROLE_GLITCH, ROLE_HYPNOTIST, ROLE_PHANTOM, ROLE_JESTER, ROLE_ZOMBIE, ROLE_VAMPIRE, ROLE_SWAPPER, ROLE_ASSASSIN, ROLE_KILLER, ROLE_LOOKOUT }
...
if CLIENT then
    ...
    function SWEP:DrawHUD()
        ...
        if self.dt.can_rag_pin and IsValid(self.dt.carried_rag) and not client:IsSpec() and (self.dt.can_rag_pin_inno or player.IsTraitorTeam(client)) then
            local tr = util.TraceLine({
                start = client:EyePos(),
